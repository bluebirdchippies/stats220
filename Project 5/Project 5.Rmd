...
I'm not sure how to reference it. But my last data set involved was from Spotify 2026. Using teh Web API on the Spotify for Developers Website. And I got meta data from my own spotify account. All data was viewed 26/27 May 2026.
...
I have mentioned this in the comments of my visualisation.R file, but the graph may look unusual because the data has been normalised. I normalised the values because the number of songs added to my playlist was much higher than the number of Beehive articles created. Without normalization, ggplot does not include some points from the playlist frequency due to how much higher it is to the article freqeuncy

Another reason the data may look uneven is because I used a left join with the Beehive dataset as the main table. This means all matching Beehive dates were kept, but some dates from the playlist dataset were not included if there was no matching Beehive article data for that date. As a result, some playlist observations were lost during the join. 
...
``` 
```{r file = 'spotify_api_scrape.R', eval=FALSE, echo=TRUE}
